...
const MobileTab = () => {
    ...
    return(
        <>
            <div className="lg:hidden bg-white pointer shadow-lg p-3 fixed bottom-0 z-10 w-full flex items-center justify-around md:justify-evenly text-gray-500 border">
                {allTypes.map((items) => (
                    <Link to={`/${items.id}`}>
                    <div
                        className={
...
const DesktopTab = () => {
    ...
    return (
      <>
        <div className="hidden lg:flex gap-14 container pointer px-20 my-8 mx-auto">
          {allTypes.map((items) => (
            <Link to={`/${items.id}`}>
              <div
...
const DesktopTab = () => {
    ...
    return(
        <>
            <div className="hidden lg:block bg-white shadow-lg p-3 fixed bottom-0 z-10 w-full flex items-center justify-around md:justify-evenly text-gray-500 border">
                {allTypes.map((items) => (
                    <Link to={`/${items.id}`}>
                    <div
                        className={
                        type === items.id
                            ? "flex flex-col my-5 relative items-center text-xl text-zomato-400 "
                            : "flex flex-col items-center text-xl "
                        }
                    >
                        <div
                        className={
                            type === items.id &&
                            "absolute -bottom-3 w-full h-2 pb-6 border-b-2 border-zomato-400"
                        }
                        />
                        {items.icon}
                        <h5 className="text-sm">{items.name}</h5>
